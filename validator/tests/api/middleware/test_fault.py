...
class FaultMiddlewareTest(ValidatorTestCase):
    ...
    def test_disguised_http_exception_with_newline(self):
        ...
        msg = wrapper._error(validator_exc.HTTPExceptionDisguise(newline_error))
        error = {'code': 400,
    ...
    def test_openstack_exception_with_kwargs(self):
        wrapper = fault.FaultWrapper(None)
        msg = wrapper._error(validator_exc.InvalidContentType(content_type='a'))
        error = {u'code': 500,
    ...
    def test_openstack_exception_without_kwargs(self):
        wrapper = fault.FaultWrapper(None)
        msg = wrapper._error(validator_exc.OpenstackException())
        error = {u'code': 500,
    ...
    def test_exception_with_non_ascii_chars(self):
        ...
        class TestException(validator_exc.OpenstackException):
            msg_fmt = msg
    ...
    def test_all_remote_exceptions(self):
        for name, obj in inspect.getmembers(
                validator_exc, lambda x: inspect.isclass(x) and issubclass(
                    x, validator_exc.OpenstackException)):

            if '__init__' in obj.__dict__:
                if obj == validator_exc.OpenstackException:  # manually ignore baseclass
                    continue
