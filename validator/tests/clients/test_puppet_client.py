...
CONF = cfg.CONF
CONF.import_group('clients_puppet', 'puppet_validator.clients.puppet_client_ssh')


class PuppetClientTestCase(tb.ValidatorTestCase):
    """Docker Client unit tests"""

    def setUp(self):
        """ Create a docker client"""
        super(PuppetClientTestCase, self).setUp()
        self.client = PuppetClient()
        CONF.set_override('cmd_test', "cmdtest {}", group='clients_puppet')
        CONF.set_override('cmd_install', "cmdinstall {}", group='clients_puppet')
        CONF.set_override('cmd_inject', "cmdinject {}", group='clients_puppet')
        CONF.set_override('cmd_launch', "cmdlaunch {}", group='clients_puppet')

    def test_create_client(self):
        """ Test client creation"""
        self.assertRaises(DockerException, PuppetClient, 'fakeurl')
    ...
    def tearDown(self):
        """ Cleanup environment"""
        super(PuppetClientTestCase, self).tearDown()
        self.m.UnsetStubs()
        self.m.ResetAll()
